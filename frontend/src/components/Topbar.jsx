...
export default function Topbar({ title }) {
  ...
  return (
    <header className="flex flex-wrap gap-3 justify-between items-center pl-14 pr-4 py-3 @min-[768px]:px-6 bg-white shadow">
      <h1 className="text-xl @min-[640px]:text-2xl @min-[768px]:text-3xl font-bold text-brand">{title}</h1>
      <div className="flex items-center gap-3 @min-[640px]:gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="hidden @min-[640px]:inline text-base font-medium text-brand">{username}</span>
        </div>
      </div>
    </header>
  );
